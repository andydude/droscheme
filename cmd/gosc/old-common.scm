...
(define (ds-function->go-mangled-name proc)
  (let ((out (ds-function-mangled-name proc)))
...
(define (ds-function->go-func proc)
  (if (ds-function-attributes-has? proc 'receiver)
...
(define (ds-function->gos-import proc)
  (let ((name  (emit-string (symbol->string (ds-function-name proc))))
        (mname (ds-function->go-mangled-name proc))
...
(define (define-func->ds-function expr)
  ...
  (let* ((r (cdr expr))
         ...
         (name (car sig))
         (sig (cdr sig)))
    (make-ds-function name (map car (most sig))
      (cons 'signature sig)
      (cons 'begin body)
      (cons 'library (current-library-name)))))
   ...
   ((pair? ls)
    (let ((fs (car ls))
      ...
      (cond
       ...
       ((pair? fs) (list fs (list->signature rs)))
       ((vector? fs)
...
(define (emit-function-name fn)
  (let ((out (emit fn)))
    (string-append
     (if (ds-function-name-public? fn) "" "_")
...
(define (emit-define-func-compile fsig . rest)
  (if (symbol? fsig)
      ...
      (let* ((fn
            ...
            (fs (symbol->string fn)))
        (string-append
         (if (ds-function-name-public? fs)
             ""
             (string-append
              "var "
              (emit-symbol fn)
              " = NewProc(" (emit-function-name fn) ", "
              (emit-string (symbol->string fn)) ")\n"))
         "func "
         (if (eqv? fs "")
             (apply emit-func (cdr fsig))
...
(define (emit-expr-case-clause clause)
  (cond
   ((eqv? (car clause) 'case)
    (string-append
     "case " (emit (cadr clause)) ":\n" 
     (list->go-block (cddr clause))))
   ((eqv? (car clause) 'default)
    (string-append
     "default:\n" 
     (list->go-block (cddr clause))))))
...
(define (emit-switch-cond* a . rest)
  (string-append
    "switch " (emit a) "; "
    (emit-braces emit-expr-case-clause rest)))
(define (emit-switch b . rest)
  (string-append
    "switch " (emit b)
    (emit-braces emit-expr-case-clause rest)))
(define (emit-switch* a b . rest)
  (string-append
...
(define (emit-package-compile name . rest)
  (apply string-append (apply emit-define-library-begin rest)))
...
(define (emit-fromto t k . r)
  (if (null? r)
      (string-append (emit t) "[" (emit k) ":]")
      (string-append (emit t) "[" (emit k) ":" (emit (car r)) "]")))
  ...
  (if (vector? arg)
      ...
      (if (pair? arg)
          (let ((vs (most arg)) (ts (last arg)))
            (string-append (list->go-list vs) " " (emit ts)))
          (if (symbol? arg)
              (emit-symbol arg)
              (error "WHAT?!?!?" arg)))))
...
(define (main-racket)
  (begin
    (*input-filename* (vector-ref (current-command-line-arguments) 0))
...
;    (cons '<-chan         emit-<-chan     )
    ...
    (cons '>              emit->          )
    (cons '>=             emit->=         )
    (cons 'adr            emit-adr        ) ; go &a
    (cons 'and            emit-and        ) ; go a && b
    (cons 'array          emit-array      ) ; go [a]b
    (cons 'array-slice    emit-slice      )
    (cons 'as             emit-as         ) ; go a.(b).c().(d).e()
...
;    (cons 'go             emit-go         )
    (cons 'goto           emit-goto       )
    (cons 'if             emit-if         )
    (cons 'if1            emit-if1        ) ; go if c {}
    (cons 'if2            emit-if2        ) ; go if stmt; c {}
    (cons 'import         emit-import     )
    (cons 'index          emit-index      ) ; go a[b]
    (cons 'index-slice    emit-fromto     ) ; go a[b]
...
(define droscheme-multiple-files (make-parameter #f))
(define droscheme-pretty-print (make-parameter #t))
