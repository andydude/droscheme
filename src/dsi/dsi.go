...
func shell() {
	...
	for line, rerr := getLine(in, prompt); rerr == nil; line, rerr = getLine(in, prompt) {
		...
		if CountParens(lines) != 0 {
			prompt = "   -"
			continue
		}

		//R
		val, lerr := ReadString(lines)
		if lerr != nil {
			fmt.Println("ReadError: " + lerr.Error())
			continue
		}
...
func main() {
	...
	if gUnmangle != "" {
		fmt.Printf("%s\n", UnmangleName(gUnmangle))
		os.Exit(0)
	}

	if gFilename != "" {
		gShell = false
		_, err := Load(gFilename, gEnv)
		if err != nil {
			panic(err)
		}
	}
