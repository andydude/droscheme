(define-library
  ...
  (begin
    ...
    (define (boolean? a) (:= (_ ok) (as a bool)) ok)
    (define (boolean=? a b)
    ...
    (define (bytevector? a)
      (:= (_ ok) (as a Binary))
      ok)
    (define (car ls) (as ls (ptr Pair) car))
    (define-func
      (car+cdr (ls Any) (car cdr Any))
      (return (car ls) (cdr ls)))
    (define (cdr ls) (as ls (ptr Pair) cdr))
    (define (char->integer ch)
      (call int (call rune (as ch Char))))
    (define (char=? a b)
      (== (as a Char) (as b Char)))
    (define (char? a) (:= (_ ok) (as a Char)) ok)
    (define (cons a b) (adr #(Pair a b)))
    (define (eof-object) gEOF)
    (define (eof-object? a)
      (if2 (:= (ch ok) (as a Char))
           ok
           (return (== ch gEOF)))
    ...
    (define (equal? a b)
      (if2 (:= (c ok) (as a Equaler))
           ok
           (return (dot c (Equal b))))
      (if2 (:= (c ok) (as b Equaler))
           ok
    ...
    (define (error-object? a)
      (:= (_ ok) (as a Error))
      ok)
    (define (exact=? a b) (object-equal? a b))
    (define (exact? a)
      (if2 (:= (num ok) (as a Num))
           ok
           (dot num (IsExact)))
      #f)
    (define (inexact=? a b) (object-equal? a b))
    (define (inexact? a)
      (if2 (:= (num ok) (as a Num))
           ok
           (dot num (IsInexact)))
    ...
    (define (last-pair ls)
      ...
      (while (pair? (cdr cur)) (= cur (cdr cur)))
      cur)
    (define (length ls)
      (if1 (null? ls) (return 0))
      (if2 (:= (_ ok) (as ls (ptr Pair)))
           ok
    ...
    (define (list* . o)
      (:= (most last)
          (inline
            "o[0:len(o) - 1], []Any(o[len(o) - 1].(Vector))"))
      (:= vs
          (call append
                most
                ((array-slice Any)
                 (as (list->vector last) Vector))))
      (vector->list (Vector vs)))
    (define (list+ a . rest)
      (:= first
          ((array-slice Any) (as (list->vector a) Vector)))
      (vector->list
        (Vector (call append first (postellipsis rest)))))
    (define (list? a) (null? (cdr (last-pair a))))
    (define (list . o) (vector->list o))
    ...
    (define (procedure? a)
      (:= (_ ok) (as a Proc))
      ok)
    (define (raise err) (call panic err) (void))
    (define (string . chars)
    ...
    (define (string->vector a)
      ...
      (for (:= i 0)
           ...
           (inline "vc[i] = st[i]"))
      vc)
    (define (string? a) (:= (_ ok) (as a String)) ok)
    (define (symbol=? a b)
      (== (as a Symbol (String))
          (as b Symbol (String))))
    ...
    (define (vector->list* a)
      (:= vc (as a Vector))
      (if1 (== (call len vc) 0) (return (null)))
      (if1 (== (call len vc) 1) (return (index vc 0)))
      (cons (index vc 0)
            (vector->list* (Vector (index-slice vc 1)))))
    ...
    (define (vector->list a)
      ...
      (for (:= i (inline "len(vc) - 1"))
           (>= i 0)
           (-- i)
           (= ls (cons (inline "vc[i]") ls)))
      ls)
    (define (vector? a) (:= (_ ok) (as a Vector)) ok)
    (define (void . o) gVoid)
    (define (void? a)
      (:= (_ ok) (as a (ptr Void)))
      ok)
    (define (u8? a)
      (if2 (:= (_ ok) (as a byte)) ok (return #t))
      (if2 (:= (c ok) (as a int))
           (and ok (<= 0 c 255))
           (return #t))
      #f)
